...
void handleGesto() {
  ...
  // --- ADICIONE ESTAS DUAS LINHAS PARA HABILITAR CORS ---
  server.sendHeader("Access-Control-Allow-Origin", "*"); // Permite requisições de qualquer origem
  server.sendHeader("Access-Control-Allow-Methods", "GET,POST,OPTIONS"); // Permite GET, POST, OPTIONS
  // Se quiser restringir a uma origem específica (mais seguro para produção):
  // server.sendHeader("Access-Control-Allow-Origin", "http://seu-dominio.com"); 
  // Mas para desenvolvimento local, "*" é o mais fácil.
  // -----------------------------------------------------

  server.send(200, "text/plain", String(letraReconhecida));
}

// ... o resto do seu código ESP32 ...
